...
export function ProductsPage({ locale, home, page, common }: { locale: Locale; home: HomeContent; page: PageContent["products"]; common: PageContent["common"] }) {
  ...
  return <>
    ...
    <section className="product-forms interior-section" aria-labelledby="other-forms-title" data-motion-chapter>
      ...
      <div className="product-forms__list">
        {alternatives.map((product, index) => <article id={`form-${product.id}`} data-motion-item key={product.id}>
          <div className="product-forms__image"><Image src={formImages[product.id]} alt={`${localizedProductLabels[product.id].name} product direction`} fill sizes="(max-width: 520px) 100vw, (max-width: 1100px) 48vw, 24vw"/></div>
          <div className="product-forms__copy"><span>0{index + 2}</span><p className="maturity">{localizedProductLabels[product.id].maturity}</p><h3>{localizedProductLabels[product.id].name}</h3><p>{localizedProductCopy[product.id]}</p><ButtonLink href={localisedHref(`/get-reflexion?form=${product.id}`, locale)} variant="secondary">{common.get}</ButtonLink></div>
        </article>)}
      </div>
    </section>
    ...
    <section className="product-compare interior-section" aria-labelledby="compare-title" data-motion-chapter>
      ...
      <div className="product-compare__scroll" data-motion-item><table><thead><tr>{page.compareHeadings.map((heading) => <th scope="col" key={heading}>{heading}</th>)}</tr></thead><tbody>{page.compareRows.map((row) => <tr key={row[0]}>{row.map((cell, index) => index === 0 ? <th scope="row" data-label={page.compareHeadings[index]} key={cell}>{cell}</th> : <td data-label={page.compareHeadings[index]} key={cell}>{cell}</td>)}</tr>)}</tbody></table></div>
    </section>
