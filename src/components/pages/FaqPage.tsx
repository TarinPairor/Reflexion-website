...
export function FaqPage({ locale, home, page, common }: { locale: Locale; home: HomeContent; page: PageContent["faq"]; common: PageContent["common"] }) {
  return <>
    <section className="faq-hero" id="top" aria-labelledby="faq-page-title" data-motion-chapter>
      <div data-motion-item><p className="eyebrow">{home.nav[3]}</p><h1 id="faq-page-title">{page.heroTitle}</h1><p>{page.heroBody}</p></div>
      <nav aria-label={locale === "zh" ? "常见问题类别" : "FAQ categories"} data-motion-item>{page.categories.map((category, index) => <a href={`#faq-category-${index + 1}`} key={category[0]}>{category[0]}</a>)}</nav>
    </section>
